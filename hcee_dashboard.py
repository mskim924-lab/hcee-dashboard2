...
# ===== Sidebar =====
with st.sidebar:
    bil_label("Control Panel", "제어 패널")
    if st.button("Generate signal · 신호 생성", use_container_width=True, type="primary"):
        generate_signal()
    auto_run = st.checkbox("Auto-run · 자동 실행")
    speed = st.slider("Speed (sec/tick) · 속도", 0.3, 3.0, 0.8, 0.1) if auto_run else 0.8
    if st.button("Reset · 초기화", use_container_width=True):
        reset_state()
        st.rerun()
    st.markdown("---")
    with st.expander("Lever Baselines · 레버 베이스라인"):
        st.markdown(f"""
...
| **TC-03** | \|PXI − EXI\| > 10 | *observability flag* — emergent from TC-01/02 |
    ...
    with st.expander("Parameter Provenance · 파라미터 산출 근거"):
        st.markdown("""
...
| TH_GAP | 10 | Demo-tuned for visibility (rulebook standard: 15) |
