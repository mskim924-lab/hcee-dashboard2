...
| **TC-03** | \|PXI − EXI\| > 15 | *emergent from TC-01/02* + joint review flag |
        """)
        st.caption(
...
| TH_GAP | 15 | Two-threshold diff + 1.5 SD margin |
...
tab_loop, tab_surveys, tab_ai, tab_learning = st.tabs([
    "거버넌스 루프 · Daily Governance Loop",
    "주간 설문 · Weekly Surveys",
    "실시간 AI · Real-time AI",
    "적응 학습 · Adaptive Learning"
])

with tab_ai:
    st.markdown("### 실시간 AI 모니터링 · Real-time AI Monitoring")
    st.caption("Operational consequence · how AI behaves under current lever settings · feeds TC-04 (Phase 2)")
...
with tab_surveys:
    st.markdown("### 주간 경험 설문 · Weekly Experience Surveys")
    st.caption("Upstream input source · where PXI/EXI signals originate · PROMs + pulse survey (Phase 2)")
    b = st.columns(4)
...
with tab_loop:
    st.markdown("### 일일 거버넌스 루프 · Daily Governance Loop  ·  **Core**")
    st.caption("Experience inputs (PXI/EXI) → TC trigger evaluation → Lever adjustment · daily cadence")
    m = st.columns(6)
    ...
    if s.history:
        st.markdown("### Governance Event Timeline")
        st.caption(
            "All ticks shown. Phase transitions are color-highlighted: "
            "🟥 pink = engagement begins (TC fires), "
            "🟧 amber = stabilizing begins (recovery confirmed), "
            "🟩 green = return to normal (cycle complete)."
        )
        ...
        cols = ["tick", "PXI", "EXI", "AAL", "HOI", "IF", "TR", "TC-01", "TC-02", "event"]
...
with tab_learning:
    st.markdown("### 적응적 학습 · Adaptive Learning")
    st.caption("Meta-level · patterns over time · TC-05 policy refinement (Phase 3)")
    d = st.columns(4)
